...
{
  [Serializable]
  public class CoordinateSystem : DenseMatrix, IEquatable<CoordinateSystem>
  {
    static readonly string _item3DPattern = Parser.Vector3DPattern.Trim('^', '$');

    private static readonly string CsPattern =
        string.Format(@"^ *o: *{{(?<op>{0})}} *x: *{{(?<xv>{0})}} *y: *{{(?<yv>{0})}} *z: *{{(?<zv>{0})}} *$",
                      _item3DPattern);


    public CoordinateSystem()
      : this(
             new Point3D(0, 0, 0), UnitVector3D.XAxis.ToVector3D(), UnitVector3D.YAxis.ToVector3D(),
             UnitVector3D.ZAxis.ToVector3D()) {}
    ...
    public CoordinateSystem(Point3D origin, Vector3D xAxis, Vector3D yAxis, Vector3D zAxis) : base(4)
    {
      this.SetColumn(0, new[]
                        {
                          xAxis.X,
                          xAxis.Y,
                          xAxis.Z,
    ...
    ////{
    ////}
    public CoordinateSystem(Matrix<double> matrix) : base(4, 4, matrix.ToColumnWiseArray())
    {
    ...
    public Vector3D OffsetToBase { get { return this.Origin.ToVector3D(); } }

    public Point3D Origin { get { return new Point3D(this.SubMatrix(0, 3, 3, 1).ToRowWiseArray()); } }

    public Vector3D XAxis { get { return new Vector3D(this.SubMatrix(0, 3, 0, 1).ToRowWiseArray()); } }

    public Vector3D YAxis { get { return new Vector3D(this.SubMatrix(0, 3, 1, 1).ToRowWiseArray()); } }

    public Vector3D ZAxis { get { return new Vector3D(this.SubMatrix(0, 3, 2, 1).ToRowWiseArray()); } }
    ...
    public Matrix<double> GetRotationSubMatrix() { return CoordinateSystem.GetRotationSubMatrix(this); }
    ...
    public CoordinateSystem Invert() { return new CoordinateSystem(this.Inverse()); }

    public static bool operator ==(CoordinateSystem left, CoordinateSystem right) { return object.Equals(left, right); }
    ...
    {
      var match = Regex.Match(s, CsPattern,
    ...
    {
      return CoordinateSystem.Rotation(av, UnitVector3D.YAxis);
    }
    ...
    {
      return CoordinateSystem.Rotation(av, UnitVector3D.XAxis);
    }


    /// <summary>
    ///   Sets to the matrix of rotation that aligns the 'from' vector with the 'to' vector.
    ///   The optional Axis argument may be used when the two vectors are perpendicular and in opposite directions to specify a
    ///   specific solution, but is otherwise ignored.
    /// </summary>
    ...
    public static CoordinateSystem RotateTo(UnitVector3D fromVector3D, UnitVector3D toVector3D,
                                            UnitVector3D? axis = null)
    {
      Matrix<double> r = Matrix3D.RotationTo(fromVector3D, toVector3D, axis);
      var coordinateSystem = new CoordinateSystem();
      CoordinateSystem cs = CoordinateSystem.SetRotationSubMatrix(r, coordinateSystem);
      return cs;
    }
    ...
    {
      return CoordinateSystem.Rotation(Angle.From(a, unit), v);
    }
    ...
    {
      return CoordinateSystem.Rotation(Angle.From(a, unit), v.Normalize());
    }
    ...
    {
      return CoordinateSystem.Rotation(angle, v.Normalize());
    }
    ...
    /// <param name="unit"></param>
    public static CoordinateSystem Rotation(double yaw, double pitch, double roll, AngleUnit unit)
    {
      var ya = Angle.From(yaw, unit);
      var ra = Angle.From(roll, unit);
      var pa = Angle.From(pitch, unit);
      return CoordinateSystem.Rotation(ya, pa, ra);
    }
    ...
    public static CoordinateSystem Rotation(Angle yaw, Angle pitch, Angle roll)
    {
      ...
      var yt = CoordinateSystem.Yaw(yaw);
      var pt = CoordinateSystem.Pitch(pitch);
      var rt = CoordinateSystem.Roll(roll);
    ...
    public static CoordinateSystem SetRotationSubMatrix(Matrix<double> r, CoordinateSystem coordinateSystem)
    {
      ...
      var cs = new CoordinateSystem(coordinateSystem.Origin, coordinateSystem.XAxis, coordinateSystem.YAxis,
    ...
    public static CoordinateSystem SetToAlignCoordinateSystems(Point3D fromOrigin, Vector3D fromXAxis,
                                                               Vector3D fromYAxis, Vector3D fromZAxis, Point3D toOrigin,
                                                               Vector3D toXAxis, Vector3D toYAxis, Vector3D toZAxis)
    {
      ...
      CoordinateSystem mcs = CoordinateSystem.CreateMappingCoordinateSystem(cs1, cs2);
      return mcs;
    }
    ...
    {
      return string.Format("Origin: {0}, XAxis: {1}, YAxis: {2}, ZAxis: {3}", this.Origin, this.XAxis, this.YAxis,
                           this.ZAxis);
    }
    ...
    {
      var v3 = Vector<double>.Build.Dense(new[]
                                          {
                                            v.X,
                                            v.Y,
                                            v.Z
    ...
    {
      var v4 = Vector<double>.Build.Dense(new[]
                                          {
                                            p.X,
                                            p.Y,
                                            p.Z,
    ...
    {
      return CoordinateSystem.Rotation(av, UnitVector3D.ZAxis);
    }
  }
}
