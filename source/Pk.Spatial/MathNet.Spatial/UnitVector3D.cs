...
namespace MathNet.Spatial.Euclidean
{
  ...
  {
    /// <summary>
    ///   Using public fields cos:
    ///   http://blogs.msdn.com/b/ricom/archive/2006/08/31/performance-quiz-11-ten-questions-on-value-based-programming.aspx
    /// </summary>
    ...
    public UnitVector3D(IEnumerable<double> data) : this(data.ToArray()) { }


    public UnitVector3D(double[] data) : this(data[0], data[1], data[2])
    {
      if (data.Length != 3)
      {
    ...
    public static UnitVector3D XAxis { get { return new UnitVector3D(1, 0, 0); } }

    public static UnitVector3D YAxis { get { return new UnitVector3D(0, 1, 0); } }

    public static UnitVector3D ZAxis { get { return new UnitVector3D(0, 0, 1); } }
    ...
    {
      var numberFormatInfo = provider != null
                               ? NumberFormatInfo.GetInstance(provider) : CultureInfo.InvariantCulture.NumberFormat;
      string separator = numberFormatInfo.NumberDecimalSeparator == "," ? ";" : ",";
      return string.Format("({0}{1} {2}{1} {3})", this.X.ToString(format, numberFormatInfo), separator,
                           this.Y.ToString(format, numberFormatInfo), this.Z.ToString(format, numberFormatInfo));
    }
    ...
    ///   A tolerance value for the dot product method.  Values below 2*Precision.DoublePrecision may
    ///   cause issues.
    /// </param>
    ...
    public static Vector3D operator /(UnitVector3D v, double d) { return new Vector3D(v.X/d, v.Y/d, v.Z/d); }

    public static bool operator ==(UnitVector3D left, UnitVector3D right) { return left.Equals(right); }

    public static bool operator ==(Vector3D left, UnitVector3D right) { return left.Equals(right); }

    public static bool operator ==(UnitVector3D left, Vector3D right) { return left.Equals(right); }
    ...
    public static double operator *(UnitVector3D left, UnitVector3D right) { return left.DotProduct(right); }

    public static Vector3D operator *(double d, UnitVector3D v) { return new Vector3D(d*v.X, d*v.Y, d*v.Z); }
    ...
    public static Vector3D operator -(UnitVector3D v) { return new Vector3D(-1*v.X, -1*v.Y, -1*v.Z); }
    ...
    public override string ToString() { return this.ToString(null, CultureInfo.InvariantCulture); }
