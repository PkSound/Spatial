...
namespace MathNet.Spatial.Euclidean
{
  ...
  public struct Point3D : IEquatable<Point3D>, IFormattable
  {
    ...
    public Point3D(IEnumerable<double> data) : this(data.ToArray()) { }


    public Point3D(double[] data) : this(data[0], data[1], data[2])
    {
      if (data.Length != 3)
      {
    ...
    {
      return new Point3D(points.Average(point => point.X), points.Average(point => point.Y),
    ...
    public static Point3D MidPoint(Point3D p1, Point3D p2) { return Point3D.Centroid(p1, p2); }
    ...
    public Point3D MirrorAbout(Plane plane) { return plane.MirrorAbout(this); }
    ...
    public static Point3D operator +(Point3D p, Vector3D v) { return new Point3D(p.X + v.X, p.Y + v.Y, p.Z + v.Z); }

    public static Point3D operator +(Point3D p, UnitVector3D v) { return new Point3D(p.X + v.X, p.Y + v.Y, p.Z + v.Z); }
    ...
    public Point3D ProjectOn(Plane plane) { return plane.Project(this); }
    ...
    public Point3D TransformBy(CoordinateSystem cs) { return cs.Transform(this); }
