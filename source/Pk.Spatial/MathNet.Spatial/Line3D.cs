...
namespace MathNet.Spatial.Euclidean
{
  ...
  public struct Line3D : IEquatable<Line3D>
  {
    ...
    /// <summary>
    ///   Computes the pair of points which represent the closest distance between this Line3D and another Line3D, with the
    ///   first
    ///   point being the point on this Line3D, and the second point being the corresponding point on the other Line3D.  If the
    ///   lines
    ///   intersect the points will be identical, if the lines are parallel the first point will be the start point of this
    ///   line.
    /// </summary>
    ...
    /// <summary>
    ///   Computes the pair of points which represents the closest distance between this Line3D and another Line3D, with the
    ///   option
    ...
    public Tuple<Point3D, Point3D> ClosestPointsBetween(Line3D other, bool mustBeOnSegments)
    {
          ...
          // If the lines aren't parallel OR it doesn't have to be constrained to the segments
      {
        ...
        if (!mustBeOnSegments) return result;
    ...
    /// <param name="mustBeOnSegment">
    ///   If true the returned point is contained by the segment ends, otherwise it can be anywhere
    ...
    public Point3D ClosestPointTo(Point3D p, bool mustBeOnSegment)
    {
      ...
      if (mustBeOnSegment)
      {
        ...
        if (dotProduct > this.Length) dotProduct = this.Length;
      }
    ...
    /// </summary>
    /// <param name="p"></param>
    /// <param name="mustStartBetweenStartAndEnd">
    ///   If false the startpoint can be on the line extending beyond the start and
    ...
    public static bool operator ==(Line3D left, Line3D right) { return left.Equals(right); }
