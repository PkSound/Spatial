...
namespace MathNet.Spatial.Euclidean
{
  ...
  public struct Vector3D : IEquatable<Vector3D>, IEquatable<UnitVector3D>, IFormattable
  {
    ...
    public Vector3D(IEnumerable<double> data) : this(data.ToArray()) { }


    public Vector3D(double[] data) : this(data[0], data[1], data[2])
    {
      if (data.Length != 3)
      {
    ...
    public string ToString(string format, IFormatProvider provider = null)
    {
      ...
      return string.Format("({1}{0} {2}{0} {3})", separator, this.X.ToString(format, numberFormatInfo),
    ...
    public static bool operator ==(Vector3D left, Vector3D right) { return left.Equals(right); }
    ...
    public static double operator *(Vector3D left, Vector3D right) { return left.DotProduct(right); }

    public static Vector3D operator *(double d, Vector3D v) { return new Vector3D(d*v.X, d*v.Y, d*v.Z); }
    ...
    public static Vector3D operator -(Vector3D v) { return v.Negate(); }
    ...
    /// <returns></returns>
    public Vector3D Rotate(UnitVector3D about, double angle, AngleUnit angleUnit)
    {
    ...
    public Vector3D ScaleBy(double scaleFactor) { return scaleFactor*this; }
